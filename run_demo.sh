...
run_one() {
  ...
  sid=$(ls -t "$LOG_DIR" 2>/dev/null | grep -v "\.log$" | head -1)
  if [[ -n "$sid" ]]; then
    echo
    echo "[demo] log     -> $log"
    echo "[demo] session -> $LOG_DIR/$sid/"
    echo "[demo] To see a node's exact rendered prompt:"
    echo "       python3 -c \"import json; print(json.load(open('$LOG_DIR/$sid/nodes/n_001.json'))['prompt_sent'])\""
  fi
}
...
case "${1:-all}" in
  ...
  wipe)
    ...
    rm -rf \
      ...
      "$LOG_DIR"
    mkdir -p "$LOG_DIR"
    echo "[demo] cleared: state/artifacts, FAISS index,"
