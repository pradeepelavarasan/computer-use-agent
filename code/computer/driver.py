...
class DriverConfig:
    ...
    bundle_id: str = ""         # used for `open -b` key-window activation before clicks
    is_electron: bool = False  # routing hint: use screencapture (cua window-shot is blank for Electron)
    max_steps: int = 12
    max_failures: int = 3
...
class VisionDriver:
    ...
    def __init__(self, cua: CuaDriver, pid: int, window_id: int, client, cfg: DriverConfig):
        ...
        self.cfg = cfg
        self.steps: list[StepRecord] = []
        self._debug_click_done = False   # write one debug crosshair per run for calibration
        self._win_pts: tuple[int, int] = (0, 0)   # target window point dims (set by resolve)

    # ── window geometry ────────────────────────────────────────────────────────

    async def _resolve_target_window(self) -> Optional[dict]:
        """Re-resolve the target window BY APP NAME (source of truth).

        Updates self.pid / self.window_id / self._win_pts to the current frontmost
        on-screen window of the target app. Guards against focus theft and stale
        ids between turns. Returns the resolve_window dict or None.
        """
        from .daemon import resolve_window
        loop = asyncio.get_event_loop()
        win = await loop.run_in_executor(None, resolve_window, self.cua, self.cfg.app_name)
        if win:
            self.pid = win["pid"]
            self.window_id = win["window_id"]
            self._win_pts = (win["w"], win["h"])
        return win

    async def _ensure_foreground(self) -> None:
        """Best-effort: bring the target app to the foreground by app name.

        Capture-by-window-id is occlusion-proof, but clicks (CGEvent to the
        screen-anchored window) are most reliable when the window is on top, so
        we activate before acting. We do not hard-fail if it stays behind.
        """
        from .applescript import _run_osascript
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None,
            lambda: _run_osascript(f'tell application "{self.cfg.app_name}" to activate'),
        )
        await asyncio.sleep(0.25)

    async def _capture_window_png(self) -> bytes:
        """Capture the target window's own pixels by CGWindowID (occlusion-proof).

        `screencapture -l<windowid>` grabs the window's composited buffer even if
        it is behind the agent's browser — so we never capture the wrong app.
        Falls back to foreground + full-screen capture only if -l yields a blank.
        """
        import tempfile, os, subprocess as _sp
        loop = asyncio.get_event_loop()

        # Re-resolve each turn so window_id/bounds are current (focus theft guard).
        await self._resolve_target_window()
        wid = self.window_id
        ...
        try:
            await loop.run_in_executor(
                None, lambda: _run(["screencapture", "-x", "-o", f"-l{wid}", tmp]),
            )
            png = Path(tmp).read_bytes() if os.path.exists(tmp) else b""
        finally:
            try:
                os.unlink(tmp)
            except Exception:
        ...
        if png and not self._is_blank(png):
            return png

        # Fallback: foreground the app, then full-screen capture (last resort).
        print("[vision] screencapture -l blank/failed — foreground + full-screen fallback")
        await self._ensure_foreground()
        tmp2 = tempfile.mktemp(suffix=".png")
        try:
            await loop.run_in_executor(
                None, lambda: _run(["screencapture", "-x", "-o", tmp2]),
            )
    ...
    async def _screenshot_annotated(
        ...
        # For Electron apps, capture the TARGET WINDOW directly by CGWindowID
        # (screencapture -l). The image IS the window — occlusion-proof (works even
        # behind the agent's browser), and coordinates are naturally window-local.
        # logical_w/h are the window's POINT dimensions (from resolve_window), so
        # _execute maps normalized LLM coords → window-local points for cua-driver click.
        window_capture = False
        if self.cfg.is_electron:
            visual_png = await self._capture_window_png()
            if self._win_pts != (0, 0):
                logical_w, logical_h = self._win_pts
                window_capture = True
            else:
            ...
            if self._is_blank(cua_png):
                # Native app with a blank cua shot — capture its window by id too.
                visual_png = await self._capture_window_png()
                if self._win_pts != (0, 0):
                    logical_w, logical_h = self._win_pts
                    window_capture = True
                ...
                # cua-driver returned a valid WINDOW-ONLY screenshot already in its
                # own coordinate space.
                visual_png = cua_png
                window_capture = True
                try:
                    _img = _PILImage.open(_BytesIO(visual_png))
                    logical_w, logical_h = _img.size
                except Exception:
                    logical_w, logical_h = 1920, 1080

        # AX element overlay only makes sense for a native cua window-shot. For the
        # window-id capture (Electron) the LLM works purely from click_bbox on the
        # window image; AX boxes would be in a different coordinate space.
        elements: list[DesktopElement] = []
        legend = ""
        if not self.cfg.is_electron and not window_capture:
            try:
                ax_resp = await _async_call(
    ...
    async def _activate_app_for_action(self) -> None:
        """Make the target app's window the KEY (active) window before a coordinate click.

        Coordinate clicks (CGEvent posted to the pid) only FOCUS a field when the
        window is key. `open -b <bundle_id>` (or `open -a <app_name>`) sends the
        activate/reopen event that reliably makes the window key — proven far more
        reliable than System Events `set frontmost`, which brings the app forward
        without making its window key (the click then lands but does not focus).
        """
        import subprocess as _sp
        loop = asyncio.get_event_loop()
        cmd = (["open", "-b", self.cfg.bundle_id] if self.cfg.bundle_id
               else ["open", "-a", self.cfg.app_name])
        await loop.run_in_executor(
            None, lambda: _sp.run(cmd, capture_output=True, timeout=4))
        await asyncio.sleep(0.5)   # settle: window becomes key before the click fires
    ...
    async def _keystroke_via_system_events(self, steps: list[dict]) -> None:
        ...
        CRITICAL: cua-driver's CGEventPostToPid key events are IGNORED by some
        native apps (e.g. Telegram) — a synthetic Return inserts a newline
        instead of triggering the app's Enter-to-send handler. System Events
        keystrokes use macOS's Accessibility synthesis path, which those apps
        DO accept as real input. We make the app frontmost via `open -b` first
        (System Events delivers to the frontmost app).
        """
        import subprocess as _sp
        from .applescript import keystroke_sequence
        loop = asyncio.get_event_loop()
        # Bring the target app frontmost/key so the keystrokes land in it.
        cmd = (["open", "-b", self.cfg.bundle_id] if self.cfg.bundle_id
               else ["open", "-a", self.cfg.app_name])
        await loop.run_in_executor(None, lambda: _sp.run(cmd, capture_output=True, timeout=4))
    ...
    @staticmethod
    def _applescript_keyname(k: str) -> str:
        """Normalize an LLM key token to the name keystroke_sequence expects."""
        kl = k.strip().lower()
        mods = {"cmd", "command", "shift", "alt", "option", "opt", "ctrl", "control", "fn"}
        if kl in mods:
            return kl
        aliases = {
        ...
        return aliases.get(kl, k)   # single letters/digits pass through unchanged
    ...
    async def _execute(
        ...
        if dtype == "click":
            ...
            # Map 0-1000 normalised LLM coordinates → WINDOW-LOCAL POINTS.
            # When the screenshot was cropped to the window, orig_w/orig_h are the
            # window's point dimensions, so (coord/1000) × window_dim gives the
            # window-local point that cua-driver's `click` documents.
            cx = ((xmin + xmax) / 2) / 1000.0 * orig_w
            cy = ((ymin + ymax) / 2) / 1000.0 * orig_h
            print(f"[vision/_execute] click_bbox → window-local ({cx:.0f}, {cy:.0f}) | window {orig_w}×{orig_h} pts")
            # First click of the run: ask cua-driver to write a crosshair PNG so we
            # can verify the coordinate space empirically (its built-in calibration).
            if not self._debug_click_done:
                self._debug_click_done = True
                dbg = None
                if self.cfg.artifacts_dir:
                    dbg = str(Path(self.cfg.artifacts_dir) / "click_calibration.png")
                try:
                    await _async_call(self.cua, "call", "click", {
                        "pid": self.pid, "window_id": self.window_id,
                        "x": cx, "y": cy,
                        **({"debug_image_out": dbg} if dbg else {}),
                    })
                    if dbg:
                        print(f"[vision/_execute] calibration crosshair → {dbg}")
                except Exception as exc:
                    print(f"[vision/_execute] debug click failed ({exc}); plain click")
                    await _async_call(self.cua, "click", self.pid, self.window_id, None, cx, cy)
            else:
                await _async_call(self.cua, "click", self.pid, self.window_id, None, cx, cy)
            record["coords"] = (cx, cy)
            ...
            # cua-driver type_text posts to the pid (CGEventPostToPid) and auto-falls
            # back to CGEvent character synthesis for Electron/WebView inputs — works
            # without focus steal and without System Events fragility.
            text = str(decision.get("value", ""))
            await _async_call(self.cua, "type_text", self.pid, text)

        elif dtype == "press_key":
            # LLM sends "Return", "Escape", "Down", or combos like "cmd+k".
            # Deliver via System Events (NOT cua-driver press_key): cua-driver's
            # CGEventPostToPid Return is ignored by native apps like Telegram
            # (inserts a newline), while System Events keystrokes trigger the
            # app's real key handler (e.g. Enter-to-send). Verified: System
            # Events Return sends a Telegram message; cua-driver Return does not.
            raw_key = str(decision.get("value", ""))
            parts = [self._applescript_keyname(p) for p in raw_key.split("+") if p.strip()]
            if not parts:
                raise ValueError("press_key with empty value")
    ...
    # ── key-name mapping (LLM vocabulary → cua-driver vocabulary) ──────────────
    ...
    _MOD_ALIASES = {
        ...
        "alt": "option", "option": "option", "opt": "option", "shift": "shift", "fn": "fn",
    }

    @classmethod
    def _cua_keyname(cls, k: str) -> str:
        kl = k.strip().lower()
        return cls._KEY_ALIASES.get(kl, kl)   # letters/digits pass through

    @classmethod
    def _cua_modifier(cls, m: str) -> str:
        return cls._MOD_ALIASES.get(m.strip().lower(), "")
    ...
    async def run(self) -> DriverResult:
        ...
        for turn in range(self.cfg.max_steps):
            ...
            action_prompt = (
                ...
                "- PREFER THE KEYBOARD. Many native apps ignore synthetic coordinate clicks. To open\n"
                "  a search field, press_key the shortcut shown in it (e.g. 'cmd+k' or 'cmd+f'), then\n"
                "  type_text the query, then 'Down'/'Return' to open the top result. To COMMIT a\n"
                "  prepared action (submit / send / confirm / save), press_key 'Return' rather than\n"
                "  clicking a small commit button — coordinate clicks are unreliable. Only use\n"
                "  click_bbox when there is no keyboard path AND a prior click was confirmed to work.\n"
                ...
                "- If a click_bbox produced NO change (VERIFY=false) once, do NOT repeat it — the app\n"
                "  likely ignores coordinate clicks; switch to the keyboard path above.\n"
                "- type_text types into the CURRENTLY FOCUSED field. It does NOT press Enter.\n"
                "- Use press_key with 'Return' to submit/send, 'Escape' to cancel, 'Tab' to move focus.\n"
            ...
            verify_prompt = (
                ...
                "CRITICAL — distinguish STAGED from COMMITTED. A goal's outcome is realized only when\n"
                "the EFFECT is visible, not when the inputs are merely prepared. Common traps: text\n"
                "typed into an input field is NOT yet submitted/sent; an item highlighted is NOT yet\n"
                "opened; a form filled is NOT yet saved. If the screenshot shows the action is set up\n"
                "but a final commit step still remains — content still sitting in an input field, or a\n"
                "submit/confirm/send control still present and unpressed — reply goal_complete=FALSE\n"
                "with a next_hint for that final commit step. Reply goal_complete=true only once the\n"
                "committed result is visible (e.g. the content has left the input field and now appears\n"
