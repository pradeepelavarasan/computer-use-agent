...
def _extract(html: str) -> str:
    ...
    try:
        ...
        for a in root.xpath("//a[@href]"):
            ...
            # Clean up the href
            dp_match = re.search(r"\/(dp|gp\/product)\/[A-Z0-9]{10}", href, re.IGNORECASE)
            if dp_match:
                href = dp_match.group(0)
            elif len(href) > 100:
                href = href.split("?")[0]
            
            # Find any image inside the link or nearby
            img_url = ""
            imgs = a.xpath(".//img")
            if imgs:
                img = imgs[0]
                img_url = img.get("data-src") or img.get("data-lazy-src") or img.get("src") or img.get("data-old-hires") or img.get("data-a-dynamic-image") or ""
            
            text_content = (a.text_content() or "").strip().replace("\n", " ")
            links_info.append(f"[Link] text='{text_content}' href='{href}' imgUrl='{img_url}'")
...
def _is_useful_extract(content: str, goal: str) -> bool:
    ...
    # For any product/listing extraction goals, check if we actually found clean links/items
    if any(w in goal.lower() for w in ("listing", "product")):
        if "=== EXTRACTED LINKS & IMAGES ===" not in content:
            return False
...
class BrowserSkill:
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        ...
        if html and force_path not in ("a11y", "vision"):
        ...
        last_url = url
        if vis_result:
            if getattr(vis_result, "steps", None):
                last_path = "vision"
                last_turns = getattr(vis_result, "turns", 0)
                last_actions = getattr(vis_result, "actions", [])
                last_url = getattr(vis_result, "final_url", url)
        elif a11y_result:
            if getattr(a11y_result, "steps", None):
                last_path = "a11y"
                last_turns = getattr(a11y_result, "turns", 0)
                last_actions = getattr(a11y_result, "actions", [])
                last_url = getattr(a11y_result, "final_url", url)

        return self._pack_error(url, goal, "interaction_failed",
                                f"all layers exhausted; last: {last_err}",
                                elapsed=time.time() - t0,
                                path=last_path, turns=last_turns,
                                actions=last_actions, final_url=last_url)
    ...
    async def _drive(self, DriverCls, url, goal, client, artifacts_dir,
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                await asyncio.sleep(1.5)  # wait for dynamic scripts and lazy images to load
    ...
    def _pack_error(self, url, goal, code, msg, *, elapsed=0.0, path="extract", turns=0, actions=None, final_url=None) -> AgentResult:
        out = BrowserOutput(
            url=url or "", goal=goal, path=path, turns=turns, content=None,
            actions=actions or [], final_url=final_url,
        )
        return AgentResult(
