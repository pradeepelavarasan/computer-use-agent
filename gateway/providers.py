...
def build_providers(cache_store):
    ...
    if keys:
        for idx, key in enumerate(keys, 1):
            out[f"gemini_lite_{idx}"] = GeminiProvider(key, os.getenv("GEMINI_LITE_MODEL", "gemini-2.5-flash-lite"), cache_store)
        out["gemini_flash"] = GeminiProvider(keys[0], os.getenv("GEMINI_FLASH_MODEL", "gemini-2.5-flash"), cache_store)
