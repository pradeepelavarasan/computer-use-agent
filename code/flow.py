...
class Graph:
    ...
    def extend_from(self, src_nid: str, result: AgentResult,
        ...
        for new_id, raw_inputs in pending:
            ...
            # Fan-out worker case or no graph node dependencies: we record the
            # structural parent edge so the executor's `ready_nodes` ordering and
            # replay/visualization topology stay coherent and connected.
            if not raw_inputs or not any(inp.startswith("n:") and inp in self.g.nodes for inp in resolved):
    ...
    async def run(self, query: str, *, session_id: str | None = None,
        ...
        try:
            ...
            if not db_path.exists():
                db_path = Path(__file__).resolve().parents[1] / "gateway" / "gateway_v9.db"
            if db_path.exists():
        ...
        print(f"\n{'═' * 78}\nFINAL: {(formatter_answer or '')[:2000]}\n{'═' * 78}\n")
...
def main() -> None:
    ...
    sid_for_log = resume_sid or f"s9-{time.strftime('%Y-%m-%d_%H-%M-%S')}"
    log_dir = Path(__file__).resolve().parents[1] / "logs" / sid_for_log
    log_dir.mkdir(parents=True, exist_ok=True)
    log_path = log_dir / "run.log"

    # Print session ID to real stdout BEFORE the pipe redirect
    ...
    # OS-level FD tee — captures uvicorn, MCP subprocesses, crawl4ai, native C libs.
    ...
    try:
        ...
        # Auto-generate customized log markdown for this session
        try:
            import subprocess
            script_path = Path(__file__).resolve().parent / "generate_custom_log.py"
            subprocess.run([sys.executable, str(script_path), "--session", sid_for_log], check=False)
        except Exception as e:
            sys.stderr.write(f"[flow] Failed to auto-generate customized log: {e}\n")
