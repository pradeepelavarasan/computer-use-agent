...
() => {
  ...
  for (const el of outermost) {
    ...
    if (imgEl) {
      imgUrl = imgEl.getAttribute('src') || imgEl.getAttribute('data-old-hires') || imgEl.getAttribute('data-a-dynamic-image') || '';
    }
    let options = '';
    if (el.tagName.toLowerCase() === 'select') {
      options = Array.from(el.options).map(o => o.text.trim()).filter(Boolean).join(' | ');
    }
    out.push({
      id: ++id,
...
class Element:
    ...
    def legend_line(self) -> str:
        """browser-use-style: `[id]<tag role="role" href="href" imgUrl="imgUrl" options="options">name</tag>`"""
        role = f' role="{self.role}"' if self.role else ""
        href_str = f' href="{self.href}"' if self.href else ""
        img_str = f' imgUrl="{self.imgUrl}"' if self.imgUrl else ""
        opts_str = f' options="{self.options}"' if self.options else ""
        name = self.name or ""
        return f"[{self.id}]<{self.tag}{role}{href_str}{img_str}{opts_str}>{name}</{self.tag}>"
