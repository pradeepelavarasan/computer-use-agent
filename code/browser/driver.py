...
SYSTEM_PROMPT_VISION = (
    ...
    "\n"
    "CRITICAL RULES:\n"
    "  - Once you have successfully applied the required filters, sorts, or navigation steps requested in the goal, you are DONE. Do NOT attempt to perform any actions to 'extract' the data yourself. The system will automatically perform the extraction from the page content once you call the `done` action. Simply call `done(success=true)` once the filters/sorts/navigation are applied.\n"
    "  - Never bundle `done` with other actions in the same turn. After a click or type, wait and observe the next turn's element list or screenshot to confirm the desired state before declaring done.\n"
    "  - The search query is already pre-loaded via the starting URL, meaning the search results page is already active from Turn 1. DO NOT type the query or click the search/Go button, even if you do not see a search action in your RECENT ACTIONS history. Re-submitting the search will clear all filters you have already applied.\n"
...
SYSTEM_PROMPT_A11Y = (
    ...
    "  - The search query is already pre-loaded via the starting URL, meaning the search results page is already active from Turn 1. DO NOT type the query or click the search/Go button, even if you do not see a search action in your RECENT ACTIONS history. Re-submitting the search will clear all filters you have already applied.\n"
    "  - If a filter, sort option, or button requested in the goal is missing or not present on the page (e.g. a specific checkbox is not found), DO NOT loop or scroll indefinitely searching for it. Simply skip that filter/action, proceed with the extraction part of the goal, and call `done(success=false)` with a note explaining what was missing.\n"
...
async def _dispatch(action: dict, page: Page, snap: PageSnapshot, goal: str = "") -> str:
    t = action.get("type", "")
    if t == "click":
        ...
        # If it is a select tag or combobox, attempt option matching selection
        if el.tag == "select" or el.role == "combobox":
            selected_option = await page.evaluate("""
                ([cx, cy, val]) => {
                    ...
                    const isMatch = (text) => {
                        ...
                        if (!t) return false;
                        if (t.length < 4) {
                            return valLower === t || valLower.split(/\s+/).includes(t);
                        }
                        if (valLower.includes(t) || t.includes(valLower)) return true;
                        if (t.includes('best seller') && valLower.includes('best seller')) return true;
                        if (t.includes('low to high') && valLower.includes('low to high')) return true;
                        if (t.includes('high to low') && valLower.includes('high to low')) return true;
                        if (t.includes('customer review') && valLower.includes('customer review')) return true;
                        if (t.includes('newest') && valLower.includes('newest')) return true;
                        return false;
                    };
                    
                    // 1. Try to click a visible custom dropdown option link in the popover
                    const allLinks = Array.from(document.querySelectorAll('a, li, [role=option], .a-dropdown-link'));
                    for (const link of allLinks) {
                        ...
                        if (text && isMatch(text)) {
                            ...
                            if (rect.width > 0 && rect.height > 0) {
                                link.click();
                                return link.innerText || text;
                            }
                        }
                    }
                    
                    // 2. Fallback to native select matching
                    let clickedEl = document.elementFromPoint(cx, cy);
                    let sel = null;
                    let curr = clickedEl;
                    while (curr && curr !== document.body) {
                        sel = curr.querySelector('select');
                        if (sel) break;
            ...
            """, [el.cx, el.cy, goal])
            if selected_option:
                return f"selected option {selected_option!r}"
    ...
    if t == "type":
        ...
        if el.tag == "select" or el.role == "combobox":
            ...
            """, [el.cx, el.cy, action.get("value", "")])
            if selected_option:
                return f"selected option {selected_option!r}"

        await page.mouse.click(el.cx, el.cy)
        if action.get("clear", True):
            # Programmatically clear the element value
            await page.evaluate("""([cx, cy]) => {
                let el = document.elementFromPoint(cx, cy);
                while (el && el.tagName.toLowerCase() !== 'input' && el.tagName.toLowerCase() !== 'textarea' && !el.isContentEditable) {
                    el = el.parentElement;
                }
                if (el) {
            ...
            # Keyboard fallback (Control+A for Windows/Linux, Meta+A for macOS)
...
class BaseDriver:
    ...
    async def step(self, turn: int) -> tuple[bool, bool, str]:
        ...
        # Programmatic guardrail: if done is bundled with other actions, remove it
        # to ensure the page has time to navigate/update before ending.
        if len(actions) > 1 and any(a.get("type") == "done" for a in actions):
            actions = [a for a in actions if a.get("type") != "done"]
        ...
        for a in actions:
            if a.get("type") == "done":
                ...
                break
            try:
                outcome = await _dispatch(a, self.page, snap, self.config.goal)
