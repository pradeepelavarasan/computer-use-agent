...
# details (bundle IDs, whether it's Electron, routing) from this registry.
# This keeps implementation concerns out of the planner prompt.
#
# IMPORTANT: `is_electron` is a ROUTING HINT only — it tells the skill to use
# screencapture (not the cua window-shot, which is blank for Electron), skip the
# Layer 2 AppleScript false-positive, and prefer Vision. It does NOT change how the
# app is launched. We deliberately do NOT request an electron_debugging_port:
# that flag forces cua-driver to RELAUNCH the app in the BACKGROUND (to inject
# --remote-debugging-port), which defeats the pre-flight foreground activation.
# CDP is unused in the vision path, so the port is pure dead weight.
_ELECTRON_APP_REGISTRY: dict[str, dict] = {
    # force_path="vision" = skip Layers 1-4 entirely; go straight to Layer 5.
    # Set for apps where AppleScript/AX produce false positives (Electron WebView).
    "whatsapp": {
        "bundle_ids": ["net.whatsapp.WhatsApp", "com.whatsapp.Desktop"],
        "is_electron": True,
        "force_path": "vision",   # Layers 1-4 give false positives; Vision is reliable
    },
    "telegram": {
    ...
    "slack":        {"bundle_ids": ["com.tinyspeck.slackmacgap"], "is_electron": True},
    "vscode":       {"bundle_ids": ["com.microsoft.VSCode"], "is_electron": True},
    "visual studio code": {"bundle_ids": ["com.microsoft.VSCode"], "is_electron": True},
    "code":         {"bundle_ids": ["com.microsoft.VSCode"], "is_electron": True},
    "cursor":       {"bundle_ids": ["com.todesktop.230313mzl4w4u92"], "is_electron": True},
    "discord":      {"bundle_ids": ["com.hnc.Discord"], "is_electron": True},
    "notion":       {"bundle_ids": ["notion.id"], "is_electron": True},
    "obsidian":     {"bundle_ids": ["md.obsidian"], "is_electron": True},
    "linear":       {"bundle_ids": ["com.linear"], "is_electron": True},
    "figma":        {"bundle_ids": ["com.figma.Desktop"], "is_electron": True},
}
...
def _enrich_app_metadata(
    ...
    """Resolve bundle_id, is_electron, and force_path from the app registry.

    The planner only needs to supply app_name. The computer skill enriches the
    metadata with technical details and routing decisions, keeping that knowledge
    here rather than in the planner prompt.
    """
    name_lower = app_name.lower().strip()
    for key, config in _ELECTRON_APP_REGISTRY.items():
        if key in name_lower or name_lower in key:
            if config.get("is_electron"):
                is_electron = True
            # Use registry force_path if caller didn't specify one
            if not force_path and "force_path" in config:
                force_path = config["force_path"]
            # Use registry bundle_id if caller didn't provide a known one
            ...
            print(f"[computer/skill] enriched {app_name!r}: "
                  f"bundle_id={bundle_id!r}, is_electron={is_electron}, "
                  f"force_path={force_path!r}")
            break
    return bundle_id, is_electron, force_path
...
class ComputerSkill:
    ...
    async def _cascade(
        ...
        is_electron: bool = bool(node.metadata.get("is_electron", False))

        # Enrich metadata from the skill's app registry — the planner only needs
        # to name the app, the skill resolves bundle_ids, is_electron, and routing.
        bundle_id, is_electron, force_path = _enrich_app_metadata(
            app_name, bundle_id, is_electron, force_path
        )
        ...
        try:
            ...
            # NOTE: no electron_debugging_port — that would relaunch the app in the
            # background (to inject --remote-debugging-port) and defeat foregrounding.
            pid, window_id = await loop.run_in_executor(
                None, lambda: _get_pid_and_window(cua, app_name, bundle_id)
            )
        ...
        # Skip for Electron apps — they have no AppleScript dictionary and produce
        # false positives (AX element count changes from UI events, not script success).
        if force_path not in ("hotkeys", "a11y", "vision") and not is_electron:
        ...
        if force_path != "vision" and not daemon_err and not is_electron:
            a11y_dir = str(Path(artifacts_dir) / "a11y") if artifacts_dir else None
            cfg = DriverConfig(
                goal=goal, app_name=app_name, bundle_id=bundle_id,
                max_steps=self.max_steps_a11y, max_failures=3,
        ...
        cfg = DriverConfig(
            goal=goal, app_name=app_name, bundle_id=bundle_id, is_electron=is_electron,
