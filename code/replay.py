...
Stdin-driven. Reads `logs/<sid>/` and walks its NodeState
...
def replay(session_id: str) -> int:
    ...
    if not states:
        print(f"replay: no nodes under logs/{session_id}/", file=sys.stderr)
        return 2
...
def main() -> int:
    ...
    if not args:
        ...
        if not sessions:
            print("replay: no sessions under logs/", file=sys.stderr)
            return 2
