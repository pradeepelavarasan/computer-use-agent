...
def render_prompt(skill: Skill, query: str, resolved: list[dict],
    ...
    parts += ["", "INPUTS:", json.dumps(resolved, indent=2, default=str)[:100_000]]
    return "\n".join(parts)
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "browser":
        ...
        metadata = node_dict.setdefault("metadata", {})
        
        # Dynamic URL and ASIN resolution for product detail pages
        shortlist_output = None
        for r in resolved:
            if r.get("kind") == "upstream" and isinstance(r.get("output"), dict):
                if "products" in r["output"]:
                    shortlist_output = r["output"]
                    break
        
        if shortlist_output:
            # Determine which product index to use
            idx = metadata.get("product_index")
            if idx is None:
                # Fallback to parsing index from label (e.g. b1, b2, b3 or n:b1)
            ...
            products = shortlist_output.get("products") or []
            if idx is not None and 0 <= idx < len(products):
                product = products[idx]
                real_url = product.get("url")
                real_asin = product.get("id")
                if real_url:
                    metadata["url"] = real_url
                    node_dict["metadata"]["url"] = real_url
                    print(f"[flow] Resolved browser node {node_id} URL dynamically: {real_url}")
                
                # Propagate ASIN to downstream analyst node
                if real_asin:
                    for other_id, other_node in graph_nodes.items():
                        if other_node.get("skill") == "product_analyst":
                            if node_id in other_node.get("inputs", []):
                                other_node_meta = other_node.setdefault("metadata", {})
                                other_node_meta["question"] = real_asin
                                print(f"[flow] Propagated ASIN {real_asin} to downstream analyst {other_id}")
